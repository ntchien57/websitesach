    ...
    public function index()
    {
        ...
        return Admin::content(function (Content $content) {
            ...
            $content->row(function ($row) {
                ...
                $totalIncome = ShopOrder::whereMonth('created_at', $monthNow)->sum('total');
                // dd($totalIncome);
                $row->column(3, new InfoBox('Chờ xác nhận', 'shopping-cart', 'aqua', '/' . config('admin.route.prefix') . '/shop_order', ShopOrder::where('status', 0)->count()));
                $row->column(3, new InfoBox('Đơn hàng đang xử lý', 'shopping-cart', 'yellow', '/' . config('admin.route.prefix') . '/shop_order', ShopOrder::where('status', 1)->count()));
        ...
        $admins = Admin::user()->roles;
            ...
            if($admins[0]['name'] === 'Administrator'){
